...
void SPIClass::begin(int8_t sck, int8_t miso, int8_t mosi, int8_t ss)
{
    ...
    if(sck == -1 && miso == -1 && mosi == -1 && ss == -1) {     //talvez possa resolver adicionando os outros pinos para HSPI PARA
        _sck = (_spi_num == HSPI) ? SCK : 18;                   //14        _sck = (_spi_num == VSPI) ? SCK : 18;    //14              
        _miso = (_spi_num == HSPI) ? MISO : 19; //12                        _miso = (_spi_num == VSPI) ? MISO : 19; //12    
        _mosi = (_spi_num == HSPI) ? MOSI : 23;//13                         _mosi = (_spi_num == VSPI) ? MOSI : 23;//13 
        _ss = (_spi_num == HSPI) ? SS : 5;//15                              _ss = (_spi_num == VSPI) ? SS : 5;    //15
    } else {
        _sck = sck;
        _miso = miso;
        _mosi = mosi;
...
SPIClass SPI; //DEVE SER PARA O ETHERNET VSPI,  MACRO EQUIVALE A 3
//SPIClass SPI(VSPI);